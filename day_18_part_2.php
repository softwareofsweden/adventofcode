...
class Day18Part2 implements ProblemInterface
{
    public function expected()
    {
        return 275011754427339;
    }

    public function doAddition($expression)
    {
        $arr = explode(' * ', $expression);
        for ($i = 0; $i < count($arr); $i++) {
            $arr[$i] = eval("return " . $arr[$i] . ";");
        }
    ...
    public function calculateSimpleExpression($expression)
    {
        if (strpos($expression, '*')) {
            $expression = $this->doAddition($expression);
        }
        $result = 0;
